...
export default function Base({ data, depth = 6 }: BaseProps) {
    ...
    data.features.forEach((feature) => {
      const polygons: RegionPolygons =
        feature.geometry.type === "Polygon"
          ? [
              feature.geometry.coordinates.map<Vector2[]>((ring) =>
                ring.map(toV2)
              ),
            ]
  ...
  useLayoutEffect(() => {
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        useAppStore.setState({ mapPlayComplete: true });
      },
    });
  ...
  return (
    <group
      ...
      <Heatmap projection={projection} position-z={depth + 1.1} />
    </group>
  );
}
